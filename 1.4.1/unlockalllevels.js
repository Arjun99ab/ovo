...
function crazyGamesLoaded() {
  ...
  crazysdk.init();
  crazysdk.addEventListener('adblockDetectionExecuted', function() {window.adblockIsEnabled = crazysdk.hasAdblock});
	crazysdk.addEventListener('adStarted', function() {c2_callFunction("muteSounds")}); // mute sound
	crazysdk.addEventListener('adFinished',  function() {c2_callFunction("adOver")}); // reenable sound, enable ui
  crazysdk.addEventListener('adError', function() {c2_callFunction("unmuteSounds")}); // reenable sound, enable ui
  // crazyMidRoll();
}
