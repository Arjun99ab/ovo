...
cr.seal(animobj);
console.log(animobj)
spritePlugin.animations[29] = animobj;		// swap array data for object

var i, len, frame;
for (i = 0, len = spritePlugin.all_frames.length; i < len; ++i)
{
  frame = spritePlugin.all_frames[i];
  frame.webGL_texture = runtime.glwrap.loadTexture(frame.texture_img, false, runtime.linearSampling, frame.pixelformat);
  if (i >= 94 && i <= 96) { //hardcode for now bc i have no idea why webgl doesnt populate the heights correctly
...
let inst = runtime.createInstance(types[0], runtime.layouts["Level 1"].layers[4], 180.5, 38); //228, -61
inst.width = 64;
inst.height = 64;
inst.set_bbox_changed();
inst.instance_vars[4] = 'Modloader'
