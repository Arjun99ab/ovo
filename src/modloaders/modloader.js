...
import {
  ...
  renderAddModMenu,
  searchMods,
} from "./util/pages/mods/render.js";
...
(function () {
  ...
  let createModLoaderMenu = () => {
      ...
      for (const [key] of Object.entries(modSettings["mods"])) {
        if (modSettings["mods"][key]["enabled"] === true) {
          if (
            ...
            !backendConfig["mods"][key]["tags"].includes("visual")
          ) {
    ...
    modsButton.onclick = function () {
      ...
      renderModsMenu(
        document.getElementById("filters-div"),
        document.getElementById("cards-div")
      );
    };
    let settingsButton = createNavButton(
      "nav-settings-btn",
      "Settings",
      "13vw"
    );
    ...
    let profilesButton = createNavButton(
      "nav-profiles-btn",
      "Profiles",
      "13vw"
    );
  ...
  let cleanModLoader = {
    async init() {
      ...
      let c = {
        backgroundColor: "rgba(150,10,1,0.8)",
        width: "5px",
        height: "5px",
        position: "absolute",
        bottom: "5px",
      ...
      if (userConfig === null) {
        ...
        for (const [key] of Object.entries(backendConfig["mods"])) {
          freshUserConfig["mods"][key] =
            backendConfig["mods"][key]["defaultSettings"];
        }
        ...
        for (const [key] of Object.entries(backendConfig["mods"])) {
          ...
          if (userConfig[key] !== undefined) {
            //old save format didn't show mods that werent on version
            freshUserConfig["mods"][key]["enabled"] =
              userConfig[key]["enabled"];
          }
        }
        ...
        for (const [key] of Object.entries(backendConfig["mods"])) {
          if (userConfig["mods"][key] === undefined) {
            ...
            console.log(key);
            if (
              backendConfig["mods"][key]["defaultSettings"]["settings"] !== null
            ) {
              let backendModSettings = Object.keys(
                backendConfig["mods"][key]["defaultSettings"]["settings"]
              );
              if (userConfig["mods"][key]["settings"] === null) {
                userConfig["mods"][key]["settings"] = {};
              }
              let userModSettings = Object.keys(
                userConfig["mods"][key]["settings"]
              );
              if (!arraysEqual(backendModSettings, userModSettings)) {
                let newMods = backendModSettings.filter(
                  (x) => !userModSettings.includes(x)
                );
                let badMods = userModSettings.filter(
                  (x) => !backendModSettings.includes(x)
                );
                for (const mod of newMods) {
                  userConfig["mods"][key]["settings"][mod] =
                    backendConfig["mods"][key]["defaultSettings"]["settings"][
                      mod
                    ];
                }
      ...
      for (const [key] of Object.entries(userConfig["mods"])) {
        ...
        if (
          !key.startsWith("custom") &&
          userConfig["mods"][key]["enabled"] === true &&
      ...
      notify(
        "QOL Modloader",
        "by Awesomeguy",
        "../src/img/modloader/modloader.png"
      );
    ...
    mouseend(event) {
      let mouse = runtime.types_by_index.find(
        (x) => x.plugin instanceof cr.plugins_.Mouse
      ).instances[0];
